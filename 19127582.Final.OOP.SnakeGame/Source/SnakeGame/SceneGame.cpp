...
void SceneGame::initializeSavedData(Coord fruit, Coord snakeHead, std::vector<Coord> snakeSegments, unsigned int round, Direction dir) {
	...
	for (auto snake : _snakes) {
		// Clear segments from the old game scene data
		snake->clearSegments();
	}
	...
	//_snakes[0]->enlonger((int) snakeSegments.size() - 2);

	for (auto i : objects) {
		if (std::dynamic_pointer_cast<SnakeSegment>(i)) _snakes[0]->addSegment(std::dynamic_pointer_cast<SnakeSegment>(i));
	}
